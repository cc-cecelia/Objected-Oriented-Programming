...
public class Library {
    ...
    public void returned(String[] info) {
        ...
        boolean expired = bookSys.checkExpired(info[1], info[3]);
        OutBook o = null;
        if (info[3].charAt(0) == 'B') {
            o = lend.returned(info, smeared, expired);
        } else if (info[3].charAt(0) == 'C') {
            if (smeared || expired) {
                lend.punished(info);
            }
    ...
    public void sort(String date) {
        ArrayList<Pair<String, String>> ans = sort.sortBooks(purchase.getPurchaseNum(), date);
        purchase.clear();
